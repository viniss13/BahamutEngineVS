...
void BHM_Component::BHM_DeleteBond(weak_ptr<BHM_Component_Queue_Bond> weakBond){
	...
	cout << "Printing size of my bonds before: " << this->rendererBonds->size() << "\n";
	for (auto bond = this->rendererBonds->begin(); bond != this->rendererBonds->end(); bond++) {
		if (weakBond.lock() == *bond) {
			cout << "Destroying bond" << endl;
			bond->reset();
			//system("pause");
			//auto bondDelete = bond;
			//bond--;
	...
	cout << "Printing size of my bonds after: " << this->rendererBonds->size() << "\n";
	/*bhm_utils::for_each_shared(this->rendererBonds, [this](auto& bond) {
		if (weakBond.lock() == *bond) {
			cout << "Destroying bond"
		}
	});*/
